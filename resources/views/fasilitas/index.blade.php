...
@section('content')
    <!-- Hero Section -->
    <section class="fasilitas-hero h-80 md:h-[400px] flex items-end relative">
    ...
    <section class="py-12 bg-[#f0f4f8]">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1 class="text-3xl md:text-4xl font-bold text-[#1e3a5f] mb-8">Fasilitas Sekolah</h1>
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition cursor-pointer"
                    ...
                    <div class="p-5">
                        ...
                        <p class="text-gray-600 text-sm mt-2">{{ Str::limit(strip_tags($item->deskripsi), 100) }}</p>
                        @if($item->jumlah_kapasitas)
                        <p class="text-xs text-gray-500 mt-3">Kapasitas: {{ $item->jumlah_kapasitas }}</p>
                        @endif
                    </div>
                </div>
                @empty
    ...
    <!-- Lightbox Modal -->
    <div id="fasilitas-lightbox" class="lightbox-overlay" onclick="closeLightbox(event)">
        <div class="lightbox-content relative">
            ...
            <button type="button" onclick="document.getElementById('lightbox-img-container').scrollBy({left: document.getElementById('lightbox-img-container').clientWidth, behavior: 'smooth'})" class="absolute right-2 top-1/2 -translate-y-1/2 text-white w-10 h-10 shadow-lg   flex items-center justify-center rounded-full opacity-70 hover:opacity-100 transition-opacity hover:bg-black/70 slider-btn slider-btn-right">&#10095;</button>
                ...
                <div class="p-6 overflow-y-auto flex-1">
                    ...
                    <p id="lightbox-kapasitas" class="text-sm font-medium text-gray-500 mb-4"></p>
...
function openLightbox(element) {
    ...
    const kapasitasEl = document.getElementById('lightbox-kapasitas');
    if (kapasitas) {
...
function closeLightbox(event, forceClose = false) {
    if (forceClose || event.target === document.getElementById('fasilitas-lightbox')) {
        document.getElementById('fasilitas-lightbox').classList.remove('active');
